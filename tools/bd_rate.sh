...
CIEDE20001=$(cut -d\  -f 8 $1 | xargs | sed 's/ /,/g')
CIEDE20002=$(cut -d\  -f 8 $2 | xargs | sed 's/ /,/g')
...
CIEDE2000_RATE=$($BJONTEGAARD 0 $N1 $AREA1 $SIZE1 $CIEDE20001 $N2 $AREA2 $SIZE2 $CIEDE20002)
CIEDE2000_DSNR=$($BJONTEGAARD 1 $N1 $AREA1 $SIZE1 $CIEDE20001 $N2 $AREA2 $SIZE2 $CIEDE20002)
...
echo "CIEDE2000" $(echo $CIEDE2000_RATE | cut -d\  -f 3) $(echo $CIEDE2000_DSNR |  cut -d\  -f 3)
